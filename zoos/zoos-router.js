  ...
  db("zoos")
    ...
    .then(zoo => {
      if (zoo.length > 0) {
        ...
        res
          .status(404)
          .json({ errorMessage: "A zoo with that ID does not exist." });
      }
    })
    .catch(() => {
...
router.post("/", (req, res) => {
  ...
  if (!zooInfo.name)
    return res
      .status(400)
      .json({ errorMessage: "Please provide the name of the zoo." });
...
router.delete("/:id", (req, res) => {
  db("zoos")
    ...
    .catch(() => {
      res
        .status(500)
        .json({ error: "There was an error while deleting the zoo." });
    });
});
...
router.put("/:id", (req, res) => {
  ...
  db("zoos")
    ...
    .catch(() => {
      res
        .status(500)
        .json({ error: "There was an error while updating the zoo." });
    });
});
